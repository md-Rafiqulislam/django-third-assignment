...
def home(request):
    ...
    def textCut(text):
        words = text.split()
        if len(words) > 10:
            newText = ' '.join(words[:10]) + '...'
        return newText

    for meal in dict['categories']:
        text = textCut(meal['strCategoryDescription'])
        textList.append(text)

    myList = []

    for i, meal in enumerate(dict['categories']):
        single_meal = {}
        for key in meal:
            single_meal[key] = meal[key]
        single_meal['text'] = textList[i]
        myList.append(single_meal)


    myDict = {'allMeals': myList}


    return render(request, 'meal/index.html', myDict )